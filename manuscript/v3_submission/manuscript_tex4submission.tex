...
reveals effects of mobility, awareness, and temperature} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
observational data \cite{Pearl_2014}. We used regional time series data
of all 401 German districts from various publicly available
...
socio-demographic variables) by 8 days (see Section Results) which
...
\cite{perkovi2015complete}). If a minimal adjustment set contained
...
\cite{mass}) for every remaining valid adjustment set as regressors and
...
increase and decrease afterwards (Fig~\ref{fig:cases-log}A). On the
...
period (Fig~\ref{fig:cases-log}B). Both figures examplify a
...
increasing (cf.~Fig~\ref{fig:timeseries}); with correlations between
...
Fig~\ref{fig:effectsrange} shows the relative causal effect of the
...
\centering
\caption{
{\bf Causal effect estimates with 99 percent confidence bands.}}
%\fontsize{8}{10}\selectfont
...
estimates include 1). Fig~\ref{fig:effectsrange} shows the effects of
...
Fig~\ref{fig:effectsrange} visualizes this relationship: For a local
outbreak with 20 daily cases as COVID-19 burden, we estimate as total
causal effect a subsequent reduction of infection rate by 11 percent.
...
perspective (Fig~\ref{fig:effectsrange}), with temperature we expect
an increase by approximately 17.8 percent at a daily average temperature
of \(0^{\circ}C\) compared to a day with \(20^{\circ}C\). For rainfall, we expect on a rainy day with 10 mm rainfall a corresponding increase of the infection rate by approximately
11.4 percent compared to a day with no precipitation.
...
\midrule
Pseudo-$R^2$ & 0.332 & 0.151 & 0.298 & 0.298 & 0.286 & 0.306 & 0.286\\
...
\cite{fowler_effect_2020, li_retrospective_2020, shi_impact_2020, Wang2020temperature}.
...
\cite{gencoglu2020causal}. There, however, they identify the causal
...
\cite{chang_modeling_2020, cowling2020impact, fowler_effect_2020, kraemer_effect_2020, lai_effect_2020, linka_outbreak_2020}.

Our findings regarding awareness/Google Trends analysis are in good
agreement with the correlations found by others \cite{effenberger_association_2020,higgins_correlations_2020,yuan_trends_2020}, who conclude that alertness to COVID-19
...
possible, in future studies \cite{higgins_correlations_2020}. As such,
...
\cite{auler_evidence_2020, xie_association_2020}. The conflicting
...
increased precipitation (rainfall) and a raise in COVID-19 cases, which supports
previous observations \cite{sobral_association_2020}.
...
(cf.~\cite{gencoglu2020causal}). As such, our proposed DAG is not
...
\cite{perkovic2017complete}. For the MAG derived from our DAG, only the
...
counts of others \cite{kraemer_effect_2020,liu_impact_2020, bannister-tyrrell_preliminary_2020, Islamm2743}.
We did not model case counts with a differential equation model like the
classic SIR-model \cite{kermack1991contributions} and its successors,
since these are more suited to prediction, e.g.~\cite{rkimodellierung}, while our choice of a negative binomial regression framework allowed us to estimate the effects of confounders
...
\cite{Openshaw1984}. The MAUP postulates that different regional
...
transmission of the SARS-CoV-2 virus \cite{world2020transmission}. In
...
weather need to be taken into account when deciding for
