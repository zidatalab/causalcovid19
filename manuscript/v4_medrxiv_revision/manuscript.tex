...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={},
            pdftitle={Causal graph analysis of COVID-19 observational data in German districts reveals effects of determining factors on new reported cases},
            colorlinks=false,
            urlcolor=blue,
            linkcolor=magenta,
            pdfborder={0 0 0}}
...
\begin{frontmatter}
  ...
  \title{Causal graph analysis of COVID-19 observational data in German districts reveals effects of determining factors on new reported cases}
...
For different official and political measures on a day and district level we used one-hot encoded daily variables, i.e.~ban of mass gatherings, school and kindergarten closures and their gradual reopening, contact restrictions, and mandatory face masks for shopping and public transport. These variables were highly correlated with each other and other variables (especially mobility). To reduce multicollinearity, we aggregated the different official/political measures by summing them into one variable ``interventions'' which describes the current severity of interventions on a given day in a given district.
...
All variables but the outcome ``Reported new cases of COVID-19'' and the offset ``Active cases'' were centered for numerical stability. We scaled continuous variables by dividing by 2 times their standard deviation, while we left binary variables as-is \citep{gelman2008scaling}. After estimating their effects, we re-scaled variables to their original scale and for mobility, we re-transformed the effects of the principal components to the original mobility variables. Additionally, we lagged the effect of all variables (but outcome, offset, and the non-dynamic socio-demographic variables) by 12 days (see Section \ref{results}) which means that we assumed that their effects on the outcome will be visible after 12 days.
...
To reduce the effects of multicollinearity and to prevent overfitting, we used a ridge regression approach \citep{ridge1970}, which is a regularization method that shrinks regression coefficients to allow for better fits on unseen data and alleviates the effect of correlated variables on their respective regression coefficients. The hyper-parameter \(\lambda\) of the ridge regression was chosen by 10-fold cross-validation, where the folds were constructed from subsets of the \(401\) districts.

We conducted a log-linear regression (function \texttt{glm} with \texttt{family=poisson()} for Poisson regression, and \texttt{glm.nb} from the \texttt{MASS} package for the negative binomial regression \citep{mass}) for the full data set to assess general model adequacy and to estimate the \(\theta\) parameter of the negative binomial. We used this parameter with the \texttt{cv.glmnet} function from the R package \texttt{glmnet} \citep{glmnet2010} with \texttt{family=negative.binomial(theta)} and chose the \(\lambda\) value within one standard deviation from the minimal \(\lambda\) as regularization hyper-parameter. Afterwards, we calculated the effects of separate exposures on the outcome. For every exposure, we analyzed the different valid adjustment sets given by analysis of the causal DAG (i.e.~the minimal and optimal adjustment sets). Then, we first checked if the respective set included unobserved variables. If this was the case for the optimal adjustment set, we discarded the unobserved variables from the set and checked if it was still a valid adjustment set (function \texttt{gac} in package \texttt{pcalg} \citep{perkovi2015complete}). If a minimal adjustment set contained unobserved variables, we discarded the whole set. If no valid adjustment set for a given exposure was available, we concluded that the effect of this exposure was unidentifiable within our causal graph. We used the function \texttt{glmnet} with the parameters \(\theta\) and \(\lambda\) as above on every remaining valid adjustment set as regressors and calculated the Akaike information criterion (AIC) for this set of regressors. Finally, for every exposure, we decided for the model/adjustment set (if available) with the lowest AIC. We report the exponentiated estimated coefficients.
...
\hspace{1em}retail and recreation & -27.70 (24.65)\\
\hspace{1em}grocery and pharmacy & -4.36 (22.96)\\
\hspace{1em}parks & 44.77 (57.40)\\
\hspace{1em}workplaces & -23.45 (20.52)\\
\hspace{1em}residential & 8.36 (6.56)\\
\hspace{1em}transit stations & -30.25 (21.31)\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Weather}}\\
\hspace{1em}Rainfall & 1.83 (3.95)\\
\hspace{1em}Temperature & 10.50 (5.15)\\
\hspace{1em}Humidity & 67.62 (13.23)\\
\hspace{1em}Wind & 3.64 (1.68)\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Policies}}\\
\hspace{1em}Interventions & 2.56 (1.33)\\
\addlinespace[0.3em]
...
\hspace{1em}Searches corona & 27.64 (18.43)\\
\hspace{1em}COVID-19 burden & 3.63 (10.49)\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Case numbers (Outcome and offset)}}\\
\hspace{1em}Reported new cases COVID-19 & 3.71 (10.52)\\
\hspace{1em}Active cases & 51.33 (123.47)\\
\bottomrule
\end{tabular}
\end{table}
...
\hspace{1em}PC1 & 0.9044 & - & -\\
\hspace{1em}PC2 & 0.9823 & - & -\\
\hspace{1em}PC3 & 1.0465 & - & -\\
\hspace{1em}PC4 & 1.0247 & - & -\\
\hspace{1em}PC5 & 1.0037 & - & -\\
\hspace{1em}PC6 & 0.9999 & - & -\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Awareness}}\\
\hspace{1em}Searches corona & 1.0101 & 1.0085 & 1.0118\\
\hspace{1em}COVID-19 burden & 0.9005 & 0.8819 & 0.9202\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Weather}}\\
\hspace{1em}Temperature & 0.9460 & 0.9129 & 0.9803\\
...
\toprule
\rotatebox{90}{Foreign citizens} & \rotatebox{90}{Mobility} & \rotatebox{90}{COVID-19 burden} & \rotatebox{90}{Searches corona} & \rotatebox{90}{Temperature} & \rotatebox{90}{Rainfall} & \rotatebox{90}{Humidity} & \rotatebox{90}{Wind} & \rotatebox{90}{ }\\
\midrule
0.005 & 0.006 & 0.002 & 0.005 & 0.004 & 0.005 & 0.006 & 0.005 & Pseudo-R2\\
- & x & x & x & x & x & x & x & \vphantom{6} Pseudo-R2\\
- & x & x & x & x & x & x & x & \vphantom{5} Pseudo-R2\\
- & x & x & x & x & x & x & x & \vphantom{4} Pseudo-R2\\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Mobility}}\\
\hspace{1em}- &  &  &  &  &  &  &  & \vphantom{5} Pseudo-R2\\
\hspace{1em}- &  &  &  &  &  &  &  & \vphantom{4} Pseudo-R2\\
\hspace{1em}- &  &  &  &  &  &  &  & \vphantom{3} Pseudo-R2\\
\hspace{1em}- &  &  &  &  &  &  &  & \vphantom{2} Pseudo-R2\\
\hspace{1em}- &  &  &  &  &  &  &  & \vphantom{1} Pseudo-R2\\
\hspace{1em}- &  &  &  &  &  &  &  & Pseudo-R2\\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Weather}}\\
\hspace{1em}- & x & x & x & x &  & x & x & Pseudo-R2\\
\hspace{1em}- & x & x & x &  &  & x &  & Pseudo-R2\\
\hspace{1em}- &  & x & x &  &  &  &  & \vphantom{1} Pseudo-R2\\
\hspace{1em}- &  & x & x & x & x & x &  & Pseudo-R2\\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Policies}}\\
\hspace{1em}- &  &  & x &  & x & x & x & \vphantom{3} Pseudo-R2\\
\hspace{1em}- &  &  & x &  & x & x & x & \vphantom{2} Pseudo-R2\\
\hspace{1em}- &  &  & x &  & x & x & x & \vphantom{1} Pseudo-R2\\
\hspace{1em}- &  &  & x &  & x & x & x & Pseudo-R2\\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Socio-demographic}}\\
\hspace{1em}- & x & x & x & x & x & x & x & \vphantom{3} Pseudo-R2\\
\hspace{1em}- & x & x & x & x & x & x & x & \vphantom{2} Pseudo-R2\\
\hspace{1em}- & x & x & x & x & x & x & x & \vphantom{1} Pseudo-R2\\
\hspace{1em}- &  & x & x &  &  &  &  & Pseudo-R2\\
\hspace{1em}- & x & x & x & x & x & x & x & Pseudo-R2\\
\hspace{1em}- &  & x &  &  &  &  &  & Pseudo-R2\\
\hspace{1em}- &  & x & x & x & x & x & x & \vphantom{1} Pseudo-R2\\
\hspace{1em}- &  & x & x & x & x & x & x & Pseudo-R2\\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Awareness}}\\
\hspace{1em}- & x &  &  & x & x & x & x & Pseudo-R2\\
\hspace{1em}- & x &  & x & x & x & x & x & Pseudo-R2\\
\bottomrule
\end{tabular}
...
Our findings suggest that the causal effects of mobility, awareness, and weather need to be taken into account when deciding for mitigation and suppression measures, depending on the recent and future COVID-19 pandemic development.
