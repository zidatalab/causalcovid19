...
mycorrs <- Modeldata %>% dplyr::select(date,contains("cases"), "Rainfall", "Temperature", "Humidity", "Wind", `Searches corona`,contains("Mobility"), `COVID-19 burden`, "Interventions") %>% 
...
knitr::kable(readr::read_csv("tables/t_descriptives.csv") %>% dplyr::select("Variable","Overall") %>% rename(`mean (SD)`=`Overall`),
  ...
  pack_rows("Policies", 12, 12) %>%
  pack_rows("Socio-demographic", 13, 22) %>%
  pack_rows("Awareness", 23, 24) %>%
  pack_rows("Case numbers (Outcome and offset)", 25, 26)
```
...
knitr::kable(causaleffects %>% 
  ...
  pack_rows("Policies", 13, 13) %>%
  pack_rows("Demographic", 14, 21)
```
...
The variable "interventions" showed ...

The effects of the different socio-demographic factors ...
...
library(knitr)
library(kableExtra)
myvariables <- readr::read_delim("tables/t_variables.csv", delim=";")[c(1:13, 18:29), ] %>% dplyr::select(variable) %>%
...
kable(myvariables %>% rename(" "=variable),
  ...
  pack_rows("Policies", 14, 14) %>%
  pack_rows("Socio-demographic", 15, 23) %>%
  pack_rows("Awareness", 24, 25) %>%
  pack_rows("Weather", 10, 13) # %>%  add_header_above(c(" " = 2, "Awareness" = 2, "Weather" = 4))
```