...
%DIF DEL ../v3_submission/manuscript_tex4submission.tex   Sat Mar 20 20:23:40 2021
%DIF ADD manuscript_tex4submission.tex                    Mon Mar 29 17:36:09 2021
...
\begin{flushleft}
{\Large
\textbf\DIFdelbegin %DIFDELCMD < \newline{Causal analysis of COVID-19 observational data in German districts
%DIFDELCMD < reveals effects of mobility, awareness, and temperature} %%%
\DIFdelend \DIFaddbegin \newline{Causal graph analysis of COVID-19 observational data in German districts reveals effects of determining factors on reported case numbers} \DIFaddend % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
\section*{Abstract}
\DIFdelbegin \DIFdel{Mobility, awareness, and weather }\DIFdelend \DIFaddbegin \DIFadd{Several determinants }\DIFaddend are suspected to be causal drivers for new cases of COVID-19 infection. Correcting for possible confounders, we estimated \DIFdelbegin \DIFdel{their causal effects }\DIFdelend \DIFaddbegin \DIFadd{the effects of the most prominent determining factors }\DIFaddend on reported case numbers. To this end, we used a directed acyclic graph (DAG) as a graphical representation of the hypothesized causal effects of the \DIFdelbegin \DIFdel{aforementioned }\DIFdelend determinants on new reported cases of COVID-19. Based on this, we computed valid adjustment sets of the possible confounding factors. We collected data for Germany from publicly available sources (e.g.~Robert Koch Institute, Germany's National Meteorological Service, Google) for 401 German districts over the period of 15 February to 8 July 2020, and estimated total causal effects based on our DAG analysis by negative binomial regression. Our analysis revealed favorable \DIFdelbegin \DIFdel{causal }\DIFdelend effects of increasing temperature, increased public mobility for essential shopping (grocery and pharmacy) \DIFaddbegin \DIFadd{or within residential areas}\DIFaddend , and awareness measured by COVID-19 burden, all of them reducing the outcome of newly reported COVID-19 cases. Conversely, we saw adverse effects \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{leading to an increase in new COVID-19 cases for }\DIFaddend public mobility in retail and recreational areas \DIFaddbegin \DIFadd{or workplaces}\DIFaddend , awareness measured by searches for ``corona'' in Google, \DIFdelbegin \DIFdel{and }\DIFdelend higher rainfall, \DIFdelbegin \DIFdel{leading to an increase in new COVID-19 cases}\DIFdelend \DIFaddbegin \DIFadd{and some socio-demographic factors. Non-pharmaceutical interventions were found to be effective in reducing case numbers}\DIFaddend . This comprehensive causal \DIFaddbegin \DIFadd{graph }\DIFaddend analysis of a variety of determinants affecting COVID-19 progression gives \DIFaddbegin \DIFadd{strong }\DIFaddend evidence for the driving forces of mobility, public awareness, and temperature, whose implications need to be taken into account for future decisions regarding pandemic management.
...
\DIFdelbegin \DIFdel{23 }\DIFdelend \DIFaddbegin \DIFadd{20 }\DIFaddend February 2020 to 8 July 2020 (\DIFdelbegin \DIFdel{\(T=137\)}\DIFdelend \DIFaddbegin \DIFadd{\(T=140\)}\DIFaddend ), since we used a lag of \DIFdelbegin \DIFdel{8
}\DIFdelend \DIFaddbegin \DIFadd{5
}\DIFaddend days for all confounders. We did not exclude any states or districts
(\(K=401\)). We analyzed the daily reported number of new cases as
outcome (\DIFdelbegin \DIFdel{\(K\cdot T=54{\ }937\) }\DIFdelend \DIFaddbegin \DIFadd{\(K\cdot T=56{\ }140\) }\DIFaddend observations). The set of possible
...
or were modelled as a construct consisting of several variables. For our causal \DIFaddbegin \DIFadd{graph }\DIFaddend analysis, we computed adjustment sets \DIFdelbegin \DIFdel{in three different
scenarios for separate }\DIFdelend \DIFaddbegin \DIFadd{separately for all observed }\DIFaddend exposures within the DAG \DIFdelbegin \DIFdel{: i) mobility of
population, ii) awareness of COVID-19 (i.e.~Google searches for
``corona''), iii) weather (i.e.~temperature}\DIFdelend \DIFaddbegin \DIFadd{(if the respective exposure was identifiable within the DAG causal analysis framework}\DIFaddend ).
...
relative change of daily time spent at residential areas. \DIFaddbegin \DIFadd{The six mobility variables showed high correlations among each other and with other variables. To reduce multicollinearity, we transformed them by principal component analysis (PCA) into six uncorrelated principal components which were used in place of the original variables.
}\DIFaddend 
...
For different official and political \DIFdelbegin \DIFdel{measures }\DIFdelend \DIFaddbegin \DIFadd{interventions on a daily basis and the district level }\DIFaddend we used one-hot encoded daily variables, i.e.~ban of mass gatherings, school and kindergarten closures and their gradual reopening, contact restrictions, and mandatory face masks for shopping and public transport.
...
%DIFDELCMD < \begin{adjustwidth}{-2.25in}{0in} % Comment out/remove adjustwidth environment if table fits in text column.
%DIFDELCMD < \centering
%DIFDELCMD < \caption{
%DIFDELCMD < {\bf Observed model variables.}}
%DIFDELCMD < \fontsize{8}{10}\selectfont
%DIFDELCMD < \begin{tabular}[t]{llll>{\raggedright\arraybackslash}p{20em}l}
%DIFDELCMD < \toprule
%DIFDELCMD < variable & dynamics & level & type & unit/comment & source\\
%DIFDELCMD < \midrule
%DIFDELCMD < Weekday & daily & national & categorical & Sat through Thu as six binary variables, Fri as baseline & -\\
%DIFDELCMD < Holiday (report) & daily & national & binary & - & -\\
%DIFDELCMD < Holiday (exposure) & daily & national & binary & - & -\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Mobility}}\\
%DIFDELCMD < \hspace{1em}retail and recreation & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \hspace{1em}grocery and pharmacy & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \hspace{1em}parks & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \hspace{1em}workplaces & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \hspace{1em}residential & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \hspace{1em}transit stations & daily & state & numeric & percent change compared to reference period & Google \cite{google_mobility}\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Weather}}\\
%DIFDELCMD < \hspace{1em}Rainfall & daily & district & numeric & mm (l/sqm) & DWD \cite{dwd_weather}\\
%DIFDELCMD < \hspace{1em}Temperature & daily & district & numeric & °C & DWD \cite{dwd_weather}\\
%DIFDELCMD < \hspace{1em}Humidity & daily & district & numeric & relative humidity (\%) & DWD \cite{dwd_weather}\\
%DIFDELCMD < \hspace{1em}Wind & daily & district & numeric & m/s & DWD \cite{dwd_weather}\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Policies}}\\
%DIFDELCMD < \hspace{1em}Ban of mass gatherings & daily & national & binary & - & -\\
%DIFDELCMD < \hspace{1em}School and kindergarten closures & daily & state & numeric & 0 for no closure, 1 for full closure, 0.5 for partial reopening & -\\
%DIFDELCMD < \hspace{1em}Contact restrictions & daily & national & binary & - & -\\
%DIFDELCMD < \hspace{1em}Mandatory face masks & daily & district & binary & - & IZA \cite{mitze2020face}\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Socio-demographic}}\\
%DIFDELCMD < \hspace{1em}Age & constant & district & numeric & 2 variables: share of population $>=65$ years \& $<18$ years & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Gender & constant & district & numeric & share of female population & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Population density & constant & district & numeric & population per sqkm & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Foreign citizens & constant & district & numeric & 2 variables: share of foreign citizens \& of population seeking refuge & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Socio-economic status & constant & district & numeric & share of households with low income & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Turnout & constant & district & numeric & voter turnout in last election & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Right-wing populist party votes & constant & district & numeric & share of votes for AfD in last election & INKAR \cite{inkar}\\
%DIFDELCMD < \hspace{1em}Nursing homes & constant & district & numeric & number of nursing (retirement) homes & INKAR \cite{inkar}\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Awareness}}\\
%DIFDELCMD < \hspace{1em}Searches corona & daily & state & numeric & percent relative to other states and observation period & Google \cite{google_trends}\\
%DIFDELCMD < \hspace{1em}COVID-19 burden & daily & district & numeric & reported cases on day of exposure & RKI \cite{casenumbers_rki}\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{6}{l}{\textbf{Case numbers}}\\
%DIFDELCMD < \hspace{1em}Reported new cases of COVID-19 & daily & district & numeric & - & RKI \cite{casenumbers_rki}\\
%DIFDELCMD < \hspace{1em}Active cases & daily & district & numeric & active cases on day of report & RKI \cite{casenumbers_rki}\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:model-vars}
...
All \DIFaddbegin \DIFadd{continuous }\DIFaddend variables but the outcome ``Reported new cases of COVID-19'' and the offset ``Active cases'' were centered \DIFaddbegin \DIFadd{and scaled by one standard deviation }\DIFaddend for numerical stability\DIFdelbegin \DIFdel{. We did
not scale variables to unit variance to maintain interpretability of effects on the original scaleof variables. Additionally}\DIFdelend \DIFaddbegin \DIFadd{, while we left binary variables as-is. After estimating the effects of variables, we re-scaled continuous variables' effects to their original scale. Additionally for mobility variables, we re-transformed the effects of the principal components to the original mobility variables. Furthermore}\DIFaddend , we lagged the effect of all variables (but outcome, offset, and the non-dynamic socio-demographic variables) by \DIFdelbegin \DIFdel{8 days (see Section Results}\DIFdelend \DIFaddbegin \DIFadd{5 days (optimal lag found by cross-validation}\DIFaddend ) which means that we assumed that their effects on the outcome will be visible after \DIFdelbegin \DIFdel{8 }\DIFdelend \DIFaddbegin \DIFadd{5 }\DIFaddend days.
...
virus, onset of COVID-19, and finally reports of COVID-19 cases. \DIFaddbegin \DIFadd{We use the terms ``causal effect'' or ``causal relationship'' for effect estimates that are based on this causal graph framework. }\DIFaddend Every
...
valid adjustment set while having the lowest \DIFdelbegin \DIFdel{asymptotic variance in the
resulting causal effect estimates}\DIFdelend \DIFaddbegin \DIFadd{Akaike information criterion (AIC)}\DIFaddend .
...
\DIFdelbegin \DIFdel{When we analyzed different }\DIFdelend \DIFaddbegin \DIFadd{We conducted a log-linear regression (function }\texttt{\DIFadd{glm}} \DIFadd{with }\texttt{\DIFadd{family=poisson()}} \DIFadd{for Poisson regression, and }\texttt{\DIFadd{glm.nb}} \DIFadd{from the }\texttt{\DIFadd{MASS}} \DIFadd{package for the negative binomial regression \mbox{%DIFAUXCMD
\cite{mass}}\hspace{0pt}%DIFAUXCMD
) for the full data set to assess general model adequacy and to estimate the \(\theta\) parameter of the negative binomial. The proper lag between exposures and outcome was found by 10-fold cross-validation on different lags between 1 and 20 days. Model diagnostics on the final full model did not show severe problems with model assumptions (linearity, distribution of residuals, independence of observations). Analysis of variance inflation factors revealed some problems with multicollinearity. To reduce the effects of multicollinearity, first we transformed the highly correlated mobility variables by PCA as described above. Second, we used a ridge regression approach \mbox{%DIFAUXCMD
\cite{ridge1970}}\hspace{0pt}%DIFAUXCMD
, which is a regularization method that shrinks regression coefficients and alleviates the effect of correlation between variables on their respective regression coefficients. Furthermore, regularized regression allows for better fits on unseen data, thus preventing overfitting the data, too. The hyper-parameter \(\lambda\) of the ridge regression was chosen by 10-fold cross-validation, where the folds were constructed from random subsets of the \(401\) districts. We used this hyper-parameter with the }\texttt{\DIFadd{cv.glmnet}} \DIFadd{function from the R package }\texttt{\DIFadd{glmnet}} \DIFadd{\mbox{%DIFAUXCMD
\cite{glmnet2010} }\hspace{0pt}%DIFAUXCMD
with }\texttt{\DIFadd{family=negative.binomial(theta)}} \DIFadd{and chose the \(\lambda\) value within one standard deviation from the minimal \(\lambda\) as regularization hyper-parameter. Afterwards, we calculated the effects of separate exposures on the outcome. For every exposure, we analyzed the different valid }\DIFaddend adjustment sets given by analysis of the causal DAG (i.e.~the minimal and optimal adjustment sets)\DIFaddbegin \DIFadd{. Then}\DIFaddend , we first checked if the \DIFaddbegin \DIFadd{respective }\DIFaddend set included unobserved variables. If this was the case for the optimal adjustment set, we discarded the unobserved variables from the set and checked if it was still a valid adjustment set (function \texttt{gac} in package \texttt{pcalg} \cite{perkovi2015complete}). If a minimal adjustment set contained unobserved variables, we discarded the whole set. \DIFdelbegin \DIFdel{We conducted a
log-linear regression (function }\texttt{\DIFdel{glm}} %DIFAUXCMD
\DIFdel{with
}\texttt{\DIFdel{family=poisson()}} %DIFAUXCMD
\DIFdel{for Poisson regression, }\DIFdelend \DIFaddbegin \DIFadd{If no valid adjustment set for a given exposure was available, we concluded that the effect of this exposure was unidentifiable within our causal graph. We used the function }\texttt{\DIFadd{glmnet}} \DIFadd{with the parameters \(\theta\) }\DIFaddend and \DIFdelbegin \texttt{\DIFdel{glm.nb}}
%DIFAUXCMD
\DIFdel{from the }\texttt{\DIFdel{MASS}} %DIFAUXCMD
\DIFdel{package for the negative binomial regression
\mbox{%DIFAUXCMD
\cite{mass}}\hspace{0pt}%DIFAUXCMD
) for }\DIFdelend \DIFaddbegin \DIFadd{\(\lambda\) as above on }\DIFaddend every remaining valid adjustment set as regressors \DIFdelbegin \DIFdel{and
calculated a Pseudo-\(R^2\) given by \(1-V_m/V_0\), where \(V_m\) isthe
...
only) . That is, our Pseudo-\(R^2\) is \(1\) minus the fraction of variance unexplained}\DIFdelend \DIFaddbegin \DIFadd{(that is, we applied ridge regression) and calculated the Akaike information criterion (AIC) for this model/set of regressors}\DIFaddend . Finally, \DIFaddbegin \DIFadd{for every exposure, }\DIFaddend we decided for the model/adjustment set \DIFdelbegin \DIFdel{with the highest pseudo-\(R^2\)}\DIFdelend \DIFaddbegin \DIFadd{(if available) with the lowest AIC}\DIFaddend . We report the exponentiated estimated coefficients \DIFdelbegin \DIFdel{along with 99 percent confidence intervals of the estimates}\DIFdelend \DIFaddbegin \DIFadd{for the separate exposures on their original scale}\DIFaddend .
...
%DIFDELCMD < \centering
%DIFDELCMD < \caption{
%DIFDELCMD < {\bf Descriptive statistics for observed variables.}}
%DIFDELCMD < %\fontsize{8}{10}\selectfont
%DIFDELCMD < \begin{tabular}[t]{ll}
%DIFDELCMD < \toprule
%DIFDELCMD < Variable & mean (SD)\\
%DIFDELCMD < \midrule
%DIFDELCMD < n & 54937\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Mobility}}\\
%DIFDELCMD < \hspace{1em}retail and recreation & -27.14 (24.59)\\
%DIFDELCMD < \hspace{1em}grocery and pharmacy & -4.04 (23.00)\\
%DIFDELCMD < \hspace{1em}parks & 46.33 (57.94)\\
%DIFDELCMD < \hspace{1em}workplaces & -22.98 (20.56)\\
%DIFDELCMD < \hspace{1em}residential & 8.20 (6.54)\\
%DIFDELCMD < \hspace{1em}transit stations & -29.83 (21.22)\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Weather}}\\
%DIFDELCMD < \hspace{1em}Rainfall & 1.85 (3.98)\\
%DIFDELCMD < \hspace{1em}Temperature & 10.74 (5.27)\\
%DIFDELCMD < \hspace{1em}Humidity & 67.68 (13.10)\\
%DIFDELCMD < \hspace{1em}Wind & 3.64 (1.67)\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Policies}}\\
%DIFDELCMD < \hspace{1em}Ban of mass gatherings & 0.82 (0.38)\\
%DIFDELCMD < \hspace{1em}School and kindergarten closures & 0.54 (0.36)\\
%DIFDELCMD < \hspace{1em}Contact restrictions & 0.74 (0.44)\\
%DIFDELCMD < \hspace{1em}Mandatory face masks & 0.48 (0.50)\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Socio-demographic}}\\
%DIFDELCMD < \hspace{1em}Age (pop. 65 and older) & 22.09 (2.74)\\
%DIFDELCMD < \hspace{1em}Age (pop. younger 18) & 16.17 (1.25)\\
%DIFDELCMD < \hspace{1em}Gender & 0.51 (0.01)\\
%DIFDELCMD < \hspace{1em}Population density & 533.75 (701.84)\\
%DIFDELCMD < \hspace{1em}Foreign citizens & 10.03 (5.14)\\
%DIFDELCMD < \hspace{1em}Foreign citizens (refugees) & 1.88 (1.14)\\
%DIFDELCMD < \hspace{1em}Socio-economic status & 30.64 (6.02)\\
%DIFDELCMD < \hspace{1em}Turnout & 75.08 (3.79)\\
%DIFDELCMD < \hspace{1em}Right-wing populist party votes & 13.39 (5.32)\\
%DIFDELCMD < \hspace{1em}Nursing homes & 36.11 (30.69)\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Awareness}}\\
%DIFDELCMD < \hspace{1em}Searches corona & 27.24 (18.31)\\
%DIFDELCMD < \hspace{1em}COVID-19 burden & 3.55 (10.37)\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{2}{l}{\textbf{Case numbers (outcome and offset)}}\\
%DIFDELCMD < \hspace{1em}Reported new cases COVID-19 & 3.60 (10.39)\\
%DIFDELCMD < \hspace{1em}Active cases & 49.83 (121.96)\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:descriptive-vars}
%DIFDELCMD < %\end{adjustwidth}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[!ht]
...
\DIFdelbegin \subsection{\DIFdel{Main results}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Main results}}\DIFaddend \label{main-results}
...
%DIFDELCMD < \centering
%DIFDELCMD < \caption{
%DIFDELCMD < {\bf Causal effect estimates with 99 percent confidence bands.}}
%DIFDELCMD < %\fontsize{8}{10}\selectfont
%DIFDELCMD < \begin{tabular}[t]{lrrr}
%DIFDELCMD < \toprule
%DIFDELCMD < cause & effect estimate & CI (lower) & CI (upper)\\
%DIFDELCMD < \midrule
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{4}{l}{\textbf{Mobility}}\\
%DIFDELCMD < \hspace{1em}retail and recreation & 1.0076 & 1.0056 & 1.0097\\
%DIFDELCMD < \hspace{1em}grocery and pharmacy & 0.9951 & 0.9938 & 0.9965\\
%DIFDELCMD < \hspace{1em}parks & 0.9998 & 0.9994 & 1.0003\\
%DIFDELCMD < \hspace{1em}transit stations & 0.9975 & 0.9946 & 1.0004\\
%DIFDELCMD < \hspace{1em}workplaces & 1.0031 & 1.0002 & 1.0059\\
%DIFDELCMD < \hspace{1em}residential & 1.0080 & 0.9987 & 1.0174\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{4}{l}{\textbf{Awareness}}\\
%DIFDELCMD < \hspace{1em}Searches corona & 1.0101 & 1.0085 & 1.0118\\
%DIFDELCMD < \hspace{1em}COVID-19 burden & 0.9942 & 0.9932 & 0.9951\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{4}{l}{\textbf{Weather}}\\
%DIFDELCMD < \hspace{1em}Temperature & 0.9918 & 0.9879 & 0.9958\\
%DIFDELCMD < \hspace{1em}Rainfall & 1.0108 & 1.0066 & 1.0150\\
%DIFDELCMD < \hspace{1em}Humidity & 1.0022 & 1.0006 & 1.0039\\
%DIFDELCMD < \hspace{1em}Wind & 0.9893 & 0.9802 & 0.9985\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:causaleffects}
%DIFDELCMD < %\end{adjustwidth}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[!ht]
...
Within our framework, we saw \DIFdelbegin \DIFdel{significant effects for mobility in
retail/recreational areas and essential shopping (grocery and pharmacy). For }\DIFdelend \DIFaddbegin \DIFadd{very different effects for individual mobility variables. For mobility in }\DIFaddend retail/recreation, an increase of 1 percent point mobility compared to the reference period (03 January to 06 February 2020) leads to an increase of the daily reported case number by about \DIFdelbegin \DIFdel{0.8 percent }\DIFdelend \DIFaddbegin \DIFadd{0.11 percent. Similarly, mobility on workplaces showed an effect of 0.33 increase in case numbers for every 1 percent point increase in mobility, while mobility on transit stations showed an effect of 0.26 increase in case numbers for every 1 percent point increase}\DIFaddend . Contrarily, \DIFdelbegin \DIFdel{a corresponding }\DIFdelend \DIFaddbegin \DIFadd{the remaining three mobility variables showed negative effects on the number of reported COVID-19 cases. An }\DIFaddend increase of 1 percent point \DIFaddbegin \DIFadd{mobility }\DIFaddend for the areas of grocery/pharmacy leads to a decrease in the reported case number by approximately \DIFdelbegin \DIFdel{0.5 percent. Mobility on workplaces showed a small effect
of 0.3 increase in case numbers for every }\DIFdelend \DIFaddbegin \DIFadd{0.23 percent, while increased mobility of 1 percent point within parks leads to a decrease in the reported case number by approximately 0.03 percent, and finally an increase of }\DIFaddend 1 percent point \DIFdelbegin \DIFdel{increase in mobility. Other causal effects of mobility were insubstantial and not
consistent in their direction (99 percentconfidence intervals of
estimates include 1)}\DIFdelend \DIFaddbegin \DIFadd{in residential mobility leads to a decrease by approximately 0.97 percent}\DIFaddend . Fig~\ref{fig:effectsrange} shows the effects of mobility on a range of possible values. Thus, we expect an increase of daily cases by approximately \DIFdelbegin \DIFdel{23 }\DIFdelend \DIFaddbegin \DIFadd{7.8 }\DIFaddend percent if mobility in \DIFdelbegin \DIFdel{retail/recreation
}\DIFdelend \DIFaddbegin \DIFadd{workplaces }\DIFaddend reaches baseline levels of 0 percent difference to the reference period. On the other hand, an increase of mobility for \DIFdelbegin \DIFdel{grocery/pharmacy }\DIFdelend \DIFaddbegin \DIFadd{residential areas }\DIFaddend by 10 percent points compared to the reference period leads to a reduction of the infection rate by approximately \DIFdelbegin \DIFdel{7 }\DIFdelend \DIFaddbegin \DIFadd{1.8 }\DIFaddend percent.

``Awareness'' had two opposite effects on the outcome in our DAG. Awareness measured by Google searches for \emph{corona} had a positive effect on the number of reported cases. An one percent point increase of the state's Google searches (relative to other states and the observation period) leads to an increase of approximately \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{0.89 }\DIFaddend percent. For example, if a district shows 10 percent points more relative searches for \emph{corona} than another one, we expect approximately \DIFdelbegin \DIFdel{11 }\DIFdelend \DIFaddbegin \DIFadd{9.3 }\DIFaddend percent more infections for this district after \DIFdelbegin \DIFdel{8 }\DIFdelend \DIFaddbegin \DIFadd{5 }\DIFaddend days. \emph{COVID-19 burden} (reported number of cases on day of exposure) affected the outcome negatively, where every additional daily case in the district leads to a \DIFdelbegin \DIFdel{0.6 }\DIFdelend \DIFaddbegin \DIFadd{0.2 }\DIFaddend percent decrease in newly reported case numbers. The \DIFdelbegin \DIFdel{last }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend plot in Fig~\ref{fig:effectsrange} visualizes this relationship: For a local outbreak with 20 daily cases as COVID-19 burden, we estimate as total causal effect a subsequent reduction of infection rate by \DIFdelbegin \DIFdel{11 }\DIFdelend \DIFaddbegin \DIFadd{3.9 }\DIFaddend percent.

Within our model, we observed \DIFdelbegin \DIFdel{a causal effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of temperature and all other weather variables. Every increase of 1 degree Celsius in temperature leads to a reduction of the daily reported case numbers by approximately \DIFdelbegin \DIFdel{0.8 }\DIFdelend \DIFaddbegin \DIFadd{0.95 }\DIFaddend percent. On the other hand, we found an increasing effect of rainfall: One millimeter (=1 liter per square meter) more rainfall leads to an increase of reported case numbers by approximately \DIFdelbegin \DIFdel{1.1 }\DIFdelend \DIFaddbegin \DIFadd{1.21 }\DIFaddend percent. We observe effects for humidity and wind as well (higher humidity \DIFdelbegin \DIFdel{leading to more cases, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend stronger wind leading to \DIFdelbegin \DIFdel{less }\DIFdelend \DIFaddbegin \DIFadd{more }\DIFaddend cases). In perspective (Fig~\ref{fig:effectsrange}), with temperature we expect an increase by approximately \DIFdelbegin \DIFdel{17.8 }\DIFdelend \DIFaddbegin \DIFadd{21 }\DIFaddend percent at a daily average temperature of \(0^{\circ}C\) compared to a day with \(20^{\circ}C\). For rainfall, we expect on a rainy day with 10 mm rainfall a corresponding increase of the infection rate by approximately \DIFdelbegin \DIFdel{11.4 }\DIFdelend \DIFaddbegin \DIFadd{12.8 }\DIFaddend percent compared to a day with no precipitation.

\DIFdelbegin \DIFdel{In all cases we }\DIFdelend \DIFaddbegin \DIFadd{The different intervention variables showed the strongest effects in our analysis, see Table~\ref{tab:causaleffects}. While the first intervention (ban of mass gatherings) reduced subsequent daily case numbers by 2.7 percent, the closure of schools/kindergartens reduced infections by an additional 7.2 percent and mandatory face masks reduced this by another 9.4 percent. The effect of contact restrictions was the strongest in our observation period, with an reduction of the case rates by 16.9.
}

\DIFadd{The effects of the different socio-demographic factors are quite small in comparison to the effects described above. We see an increasing effect on case numbers by additional nursing homes between districts. Districts with a younger population, more foreign citizens, higher population density and a lower average social-economic status showed higher case numbers, too.
}

\DIFadd{For all exposures, our analysis pipeline }\DIFaddend opted to use the \DIFdelbegin \DIFdel{reduced }\DIFdelend \DIFaddbegin \DIFadd{(reduced) }\DIFaddend optimal adjustment set over the minimal adjustment sets because of \DIFdelbegin \DIFdel{higher pseudo-\(R^2\) values (mostly
above 0.3)}\DIFdelend \DIFaddbegin \DIFadd{lower AICs}\DIFaddend , except for \DIFdelbegin \DIFdel{mobility, where }\DIFdelend \DIFaddbegin \DIFadd{exposure variable ``nursing homes'', for which }\DIFaddend the minimal adjustment set had \DIFdelbegin \DIFdel{a
...
variables (cf. ~}\DIFdelend \DIFaddbegin \DIFadd{the lowest AIC. For an overview of all final adjustments sets, see }\DIFaddend Table~\ref{tab:adjsets}\DIFdelbegin \DIFdel{, with the exception being
COVID-19 burden).
We also decided for the lag of 8 }\DIFdelend \DIFaddbegin \DIFadd{. We found that there were no valid adjustment sets for the non-identifiable variables turnout and right-wing populist party votes.
}

\DIFadd{We decided for a lag of 5 }\DIFaddend days based on \DIFdelbegin \DIFdel{the
highest pseudo-\(R^2\) values compared to other lags on the chosen
adjustment sets}\DIFdelend \DIFaddbegin \DIFadd{cross-validation}\DIFaddend . Similarly, negative binomial regression was chosen over Poisson regression, because the latter showed overdispersion and \DIFdelbegin \DIFdel{overall
lower pseudo-\(R^2\) values}\DIFdelend \DIFaddbegin \DIFadd{an higher AIC value}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{table}[!ht]
%DIFDELCMD < \begin{adjustwidth}{-2.25in}{0in} % Comment out/remove adjustwidth environment if table fits in text column.
%DIFDELCMD < \centering
%DIFDELCMD < \caption{
%DIFDELCMD < {\bf Final adjustment sets for causal analysis.}}
%DIFDELCMD < %\fontsize{8}{10}\selectfont
%DIFDELCMD < \begin{tabular}[t]{llllllll}
%DIFDELCMD < \toprule
%DIFDELCMD < \rotatebox{90}{ } & \rotatebox{90}{Mobility} & \rotatebox{90}{COVID-19 burden} & \rotatebox{90}{Searches corona} & \rotatebox{90}{Temperature} & \rotatebox{90}{Rainfall} & \rotatebox{90}{Humidity} & \rotatebox{90}{Wind}\\
%DIFDELCMD < \midrule
%DIFDELCMD < Pseudo-$R^2$ & 0.332 & 0.151 & 0.298 & 0.298 & 0.286 & 0.306 & 0.286\\
%DIFDELCMD < Weekday & x & x & x & x & x & x & x\\
%DIFDELCMD < Holiday (report) & x & x & x & x & x & x & x\\
%DIFDELCMD < Holiday (exposure) & x & x & x & x & x & x & x\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{8}{l}{\textbf{Mobility}}\\
%DIFDELCMD < \hspace{1em}retail and recreation &  &  &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}grocery and pharmacy &  &  &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}parks &  &  &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}workplaces &  &  &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}residential &  &  &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}transit stations &  &  &  &  &  &  & \\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{8}{l}{\textbf{Weather}}\\
%DIFDELCMD < \hspace{1em}Rainfall & x & x & x & x &  & x & x\\
%DIFDELCMD < \hspace{1em}Temperature & x & x & x &  &  & x & \\
%DIFDELCMD < \hspace{1em}Humidity &  & x & x &  &  &  & \\
%DIFDELCMD < \hspace{1em}Wind &  & x & x & x & x & x & \\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{8}{l}{\textbf{Policies}}\\
%DIFDELCMD < \hspace{1em}Ban of mass gatherings & x &  & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}School and kindergarten closures & x &  & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Contact restrictions & x &  & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Mandatory face masks &  &  & x & x & x & x & x\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{8}{l}{\textbf{Socio-demographic}}\\
%DIFDELCMD < \hspace{1em}Age & x & x & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Gender & x & x & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Population density & x & x & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Foreign citizens &  & x & x &  &  &  & \\
%DIFDELCMD < \hspace{1em}Socio-economic status & x & x & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Turnout &  & x &  &  &  &  & \\
%DIFDELCMD < \hspace{1em}Right-wing populist party votes &  & x & x & x & x & x & x\\
%DIFDELCMD < \hspace{1em}Nursing homes &  & x & x & x & x & x & x\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{8}{l}{\textbf{Awareness}}\\
%DIFDELCMD < \hspace{1em}Searches corona & x &  &  & x & x & x & x\\
%DIFDELCMD < \hspace{1em}COVID-19 burden & x &  & x & x & x & x & x\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tab:adjsets}
%DIFDELCMD < \end{adjustwidth}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[!ht]
...
Our objective was to identify \DIFdelbegin \DIFdel{causal effects }\DIFdelend \DIFaddbegin \DIFadd{effects of determining factors }\DIFaddend for COVID-19 cases \DIFaddbegin \DIFadd{within a causal framework}\DIFaddend . We found that weather affects the reported number of infections, especially temperature (which has a reducing effect on case numbers) and rainfall (which increases case numbers). We saw that reports of high case numbers in districts led to a reduction in new infection numbers, which indicates risk-averse awareness in the population and/or effective public health measures to suppress a local outbreak. \DIFdelbegin \DIFdel{The overall effect
of mobility showed no consistent effect, however, in specific areas
significant causal effectscould be measured}\DIFdelend \DIFaddbegin \DIFadd{Mobility showed distinct effects}\DIFaddend : Increasing activity in retail and recreational areas\DIFaddbegin \DIFadd{, as well as transit stations and workplaces }\DIFaddend increased reported case numbers, while increased movement for essential shopping (grocery and pharmacy) \DIFaddbegin \DIFadd{and in parks or residential areas }\DIFaddend led to reduced case numbers. \DIFaddbegin \DIFadd{All interventions considered (ban of mass gatherings, school/kindergarten closures, contact restrictions, mandatory face masks) reduced case numbers considerably. Socio-demographic variables had small effects individually, but in conjunction they explained larger case numbers in (urban) areas with younger population, lower socio-economic status, and higher population density.
}\DIFaddend 
...
Most research on determinants affecting case numbers of COVID-19 is restricted to single aspects \cite{fowler_effect_2020, li_retrospective_2020, shi_impact_2020, Wang2020temperature}. To reliably identify \DIFdelbegin \DIFdel{causal }\DIFdelend \DIFaddbegin \DIFadd{(causal) }\DIFaddend drivers, one must adjust for confounders. To this end, we used an integrated model with variables from different aspects like mobility, awareness, weather, or socio-demographics and identified confounders by causal analysis with a directed acyclic graph. A causal approach is used in another current COVID-19 analysis \cite{gencoglu2020causal}. There, however, they identify the causal relationships (reconstruct a DAG), while we estimated \DIFdelbegin \DIFdel{causal }\DIFdelend effects for a given hypothesized \DIFaddbegin \DIFadd{causal }\DIFaddend DAG.
...
We observed overdispersion and a substantial increase in model performance with a negative binomial regression compared to Poisson regression, which is in line with the results on COVID-19 daily case counts of \DIFdelbegin \DIFdel{others \mbox{%DIFAUXCMD
\cite{kraemer_effect_2020,liu_impact_2020, bannister-tyrrell_preliminary_2020, Islamm2743}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kraemer_effect_2020} }\hspace{0pt}%DIFAUXCMD
and others \mbox{%DIFAUXCMD
\cite{liu_impact_2020, bannister-tyrrell_preliminary_2020, Islamm2743}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . We did not model case counts with a differential equation model like the classic SIR-model \cite{kermack1991contributions} and its successors, since these are more suited to prediction \DIFdelbegin \DIFdel{, }\DIFdelend e.g.~\cite{rkimodellierung}, while our choice of a negative binomial regression framework allowed us to estimate the effects of confounders more reliably. There are more advanced statistical methods for count data, e.g.~zero-inflated models and mixed models. We tested both approaches as extensions to the negative binomial regression and experienced numerical problems and increased computing time, along with an insubstantial increase in model performance. Furthermore, our model assumed that all variables have effects proportional to the size of their measurements. It is possible that some variables show saturation effects or opposite effects for low, medium, or high values. This could be \DIFdelbegin \DIFdel{modelled }\DIFdelend \DIFaddbegin \DIFadd{modeled }\DIFaddend with polynomial or other transformations of the variables, which we did not employ due to limited temporal and spatial data availability. \DIFaddbegin \DIFadd{Interaction effects of variables and confounding effects or mediating variables are explicitly taken care of by deriving the valid adjustment sets for a given exposure based on the causal DAG. }\DIFaddend Use of a fixed DAG with effect estimation via regression assumes that data was generated by the same underlying process for the observation period. By inclusion of the successive mitigation \DIFdelbegin \DIFdel{policies
}\DIFdelend \DIFaddbegin \DIFadd{interventions }\DIFaddend as binary variables we were able to explain some of the variance caused by the changing dynamics of case numbers (similar to \cite{Islamm2743}). \DIFaddbegin \DIFadd{While multicollinearity of variables poses less of a problem for a proper causal graph analysis \mbox{%DIFAUXCMD
\cite{schisterman2017collinearitycausal}}\hspace{0pt}%DIFAUXCMD
...
In our analysis, the adverse \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of mobility in retail/recreation and \DIFaddbegin \DIFadd{workplaces and }\DIFaddend the favorable effect of mobility in grocery/pharmacy \DIFdelbegin \DIFdel{indicate that
policies }\DIFdelend \DIFaddbegin \DIFadd{and residential areas indicate that interventions }\DIFaddend like contact restrictions which limit the number of individual interactions can lead to reduced infection numbers. This is due to retail/recreational \DIFaddbegin \DIFadd{and workplace }\DIFaddend areas encompassing mostly places of \DIFdelbegin \DIFdel{social
gatherings like restaurants and bars, malls, sports and music venues,
among others}\DIFdelend \DIFaddbegin \DIFadd{(social) gatherings}\DIFaddend , while if people are doing more of their essential shopping at supermarkets \DIFdelbegin \DIFdel{, they will most likely }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend stay at home with less contact to other people\DIFaddbegin \DIFadd{, they are less likely to come in contact with infected individuals}\DIFaddend .

The \DIFdelbegin \DIFdel{causal }\DIFdelend effects of awareness measured via searches for ``corona'' and the COVID-19 burden are harder to interpret. We assume that within our model, the searches for ``corona'' are an insufficient proxy for awareness, while the decreasing effect for future case numbers of high daily COVID-19 burden indicates it affects individual risk-behavior and entails effective non-pharmaceutical interventions.
...
\DIFaddbegin \DIFadd{Furthermore, our analyses strongly support the effectiveness of non-pharmaceutical interventions. To a lesser extent, the adverse effects of some socio-demographic factors might help to identify areas that are at higher risk of local COVID-19 outbreaks.
}

\DIFaddend \section*{Conclusion}\label{conclusions}

To the best of our knowledge, this is the most comprehensive analysis of causes for COVID-19 infections which integrates different data sources (all publicly available). Causal reasoning with a DAG allows us to estimate the \DIFaddbegin \DIFadd{possible }\DIFaddend causal effects more reliably.

Our findings suggest that the \DIFdelbegin \DIFdel{causal }\DIFdelend \DIFaddbegin \DIFadd{infection-driving }\DIFaddend effects of mobility, awareness, and weather \DIFaddbegin \DIFadd{(and to some extent socio-demographic factors) }\DIFaddend need to be taken into account when deciding for mitigation and suppression \DIFdelbegin \DIFdel{measures}\DIFdelend \DIFaddbegin \DIFadd{interventions}\DIFaddend , depending on the recent and future COVID-19 pandemic development.
...
We are thankful for feedback from Thomas Czihal, Johannes Textor, \DIFdelbegin \DIFdel{and
}\DIFdelend Ralph Brinks, \DIFaddbegin \DIFadd{and an anonymous reviewer }\DIFaddend who gave helpful suggestions on \DIFdelbegin \DIFdel{an earlier draft }\DIFdelend \DIFaddbegin \DIFadd{earlier versions }\DIFaddend of the manuscript.
