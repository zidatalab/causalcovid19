...
%DIF ADD manuscript_tex4submission.tex                    Mon Mar 22 09:20:00 2021
...
or were modelled as a construct consisting of several variables. For our causal \DIFdelbegin \DIFdel{analysis}\DIFdelend \DIFaddbegin \DIFadd{graph analysis of effects}\DIFaddend , we computed adjustment sets \DIFdelbegin \DIFdel{in three different
...
resulting causal effect estimates}\DIFdelend \DIFaddbegin \DIFadd{AIC}\DIFaddend .
...
Within our model, we observed \DIFdelbegin \DIFdel{a causal effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of temperature and all other weather variables. Every increase of 1 degree Celsius in temperature leads to a reduction of the daily reported case numbers by approximately \DIFdelbegin \DIFdel{0.8 }\DIFdelend \DIFaddbegin \DIFadd{0.95 }\DIFaddend percent. On the other hand, we found an increasing effect of rainfall: One millimeter (=1 liter per square meter) more rainfall leads to an increase of reported case numbers by approximately \DIFdelbegin \DIFdel{1.1 }\DIFdelend \DIFaddbegin \DIFadd{1.21 }\DIFaddend percent. We observe effects for humidity and wind as well (higher humidity leading to more cases, stronger wind leading to \DIFdelbegin \DIFdel{less }\DIFdelend \DIFaddbegin \DIFadd{more }\DIFaddend cases). In perspective (Fig~\ref{fig:effectsrange}), with temperature we expect an increase by approximately \DIFdelbegin \DIFdel{17.8 }\DIFdelend \DIFaddbegin \DIFadd{21 }\DIFaddend percent at a daily average temperature of \(0^{\circ}C\) compared to a day with \(20^{\circ}C\). For rainfall, we expect on a rainy day with 10 mm rainfall a corresponding increase of the infection rate by approximately \DIFdelbegin \DIFdel{11.4 }\DIFdelend \DIFaddbegin \DIFadd{12.8 }\DIFaddend percent compared to a day with no precipitation.
...
above 0.3)}\DIFdelend \DIFaddbegin \DIFadd{lower AICs}\DIFaddend , except for \DIFdelbegin \DIFdel{mobility}\DIFdelend \DIFaddbegin \DIFadd{exposure variable ``nursing homes''}\DIFaddend , where the minimal adjustment set had a \DIFdelbegin \DIFdel{higher pseudo-\(R^2\). Notably, these sets always include most of our
...
COVID-19 burden).
We also decided for the lag of 8 }\DIFdelend \DIFaddbegin \DIFadd{lower AIC. We found that there were no valid adjustment sets for the non-identifiable variables turnout and right-wing populist party votes.
}
...
\DIFaddbegin \DIFadd{Furthermore, our analyses support the effectiveness of non-pharmaceutical interventions. To a lesser extent, the adverse effects of some socio-demographic factors might help to identify areas that are at higher risk of local COVID-19 outbreaks.
}
