...
trends_alle_bl <- read_delim(paste0(mydatapath, "trends_alle_bl.csv"), 
                             ";", escape_double = FALSE, col_types = cols(BL_ID = col_integer(), 
                                                                          Coronatrend = col_double(), Datum = col_date(format = "%Y-%m-%d")), 
                             locale = locale(encoding = "UTF-8"),
                             trim_ws = TRUE)
relativ_gesamt_2020_03_13 <- read_csv(paste0(mydatapath, "relativ_gesamt_2020-03-13.csv"))

relativ_bl_2020_03_13 <- left_join(trends_alle_bl %>% filter(Datum=="2020-03-13"), relativ_gesamt_2020_03_13, by="Bundesland") %>%
...
relativ_bl <- left_join(trends_alle_bl, relativ_bl_2020_03_13, by="BL_ID") %>%
